...
func (h *getPolicyResolve) Handle(params GetPolicyResolveParams) middleware.Responder {
	...
	if d.conf.EnablePolicy == endpoint.NeverEnforce {
		...
		// traffic between these sets of labels.
		if ! (d.GetPolicyRepository().GetRulesMatching(labels.NewSelectLabelArrayFromModel(params.IdentityContext.From)) ||
		d.GetPolicyRepository().GetRulesMatching(labels.NewSelectLabelArrayFromModel(params.IdentityContext.To)) ) {
			return NewGetPolicyResolveOK().WithPayload( &models.PolicyTraceResult {
				Verdict: api.Allowed.String(),
			})
		}
